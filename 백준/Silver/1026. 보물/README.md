...
### 출력 
 ...
 ---
 ## 처음에는 문제에서는 B배열의 수를 재별하면 안된다고 나와있어서, A배열을 전부 한번씩 모든 경우의 수를 다 해봐야 하고 생각했지만,
 ## 다시 보니, 둘중 한 배열을 내림차순으로, 나머지 배열을 오름차순으로 정렬해놓고 곱하면 최소값이 나오게 된다.
 ## 문제를 하나에 집중해서 생각하기보단, 여러 방면으로 생각을 넓혀서 시도해보면 좋을것 같다.
